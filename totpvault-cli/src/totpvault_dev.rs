...
impl TotpvaultDev {
    ...
    fn send_message_verify<T: Serialize + Debug + Message>(dev_path: &str, message: T, command: u8) -> Result<(), String> {
    ...
    pub fn unlock_vault(dev_path: &str, password: &str) -> Result<(), String> {
        match Self::send_message_verify(dev_path, UnlockMsg{password: password.to_string()}, CMD_UNLOCK_VAULT) {
            Ok(_) => Ok(()),
            Err(_) => Err("Failed to unlock the vault! Bad password?".to_string())
        }
    }
